...
De seguida, procedeu-se à logaritmização dos dados, uma prática comum no tratamento de dados de expressão genética para posterior análise da sua diferenciação. A logaritmização é essencial para reduzir o enviesamento dos dados.
...
# Configurar a ligação ao Ensembl
ensembl = useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
ensembl_dataset = useDataset('hsapiens_gene_ensembl', mart = ensembl)

# Remover os números de versão dos IDs dos genes Ensembl em logcounts
ensembl_ids = gsub("\\..*", "", rownames(logcounts))
rownames(logcounts) <- ensembl_ids

# Recuperar símbolos de genes para os IDs do Ensembl
gene_symbols = getBM(attributes = c("ensembl_gene_id", "external_gene_name"),
                      filters = "ensembl_gene_id",
                      values = ensembl_ids,
                      mart = ensembl)

# Percorrer cada ID do gene Ensembl em seqdata
for (i in seq_along(ensembl_ids)) {
  # Encontrar o índice do ID do gene Ensembl em gene_symbols
  idx = match(ensembl_ids[i], gene_symbols$ensembl_gene_id)
  # Se for encontrado um símbolo de gene correspondente e não for uma cadeia vazia, substitui o nome da linha
...
resultados_id_PP <- grch38 %>%
  dplyr::filter(ensgene %in% results_PP_mod$row_names) %>%
  # Select only the 'entrez' column
  dplyr::select(symbol)
resultados_id_TRU <- grch38 %>%
  dplyr::filter(ensgene %in% results_TRU_mod$row_names) %>%
  # Select only the 'entrez' column
  dplyr::select(symbol)
...
sample_id_meta = meta$sample_id

# Encontrar as correspondências entre os sample_id em data_normal e meta
indices_correspondentes = match(nomes_colunas, sample_id_meta)
...
Todos os genes, exceto um, afetam a variabilidade à esquerda do gráfico PCA 2D.

Vamos analisar os genes através dos seus IDs.

```{r}
top_10_genes <- gsub("\\..*", "", top_10_genes)


geneIDs = ensembldb::select(EnsDb.Hsapiens.v79, keys= top_10_genes, keytype = "GENEID", columns = c("SYMBOL","GENEID"))
...
Foi então, em primeiro lugar, calculada a matriz de distâncias dos nossos dados de forma a medir a diferença entre cada par de observações.Isto permitirá entender melhor as relações entre os pontos de dados mais adiante.

```{r}
#cálculo da matriz de distâncias 
dist_data = dist(data_normal, method = "euclidean")
```

```{r}
#Guarda o ficheiro gerado anteriormente
saveRDS(dist_data, file = './resultados/dist_data.rds')

dist_data <- readRDS('./resultados/dist_data.rds')
...
```{r}
cmd.mds  = cmdscale(dist_data, 3, eig = TRUE)
```

```{r}
#Guarda o ficheiro gerado anteriormente
saveRDS(cmd.mds, file = './resultados/cmd_mds.rds')

# Carregamento do ficheiro RDS do resultado do cmdscale
cmd.mds <- readRDS('./resultados/cmd_mds.rds')
```
...
O K-means clustering procura realizar a dividisão do conjunto de dados em K subgrupos distintos e não sobrepostos (clusters) previamente definidos, onde cada ponto de dados pertence apenas a um grupo. Pretende-se que os pontos de dados intra-cluster sejam o mais semelhantes possível, ao mesmo tempo em que mantém os clusters o mais diferentes (distantes) possível. São atribuídos pontos de dados a um cluster de forma que a soma das distâncias quadradas entre os pontos de dados e o centróide do cluster (média aritmética de todos os pontos de dados que pertencem a esse cluster) seja mínima. Quanto menos variação tivermos dentro dos clusters, mais homogéneos (semelhantes) serão os pontos de dados dentro do mesmo cluster. O objetivo final é encontrar grupos que capturem a estrutura subjacente dos dados e sejam interpretáveis e úteis para a análise [@dabbura-2022].

Utilizando o algoritmo K-means para agrupar dados de uma análise multidimensional em três clusters, foi criado um gráfico de dispersão para visualizar os clusters, com cores distintas para cada grupo e elipses de dispersão para representar a distribuição dos pontos. Os rótulos dos pontos mostram os valores de expressão associados a cada ponto.
...
mds_df = as.data.frame(cmd.mds)
mds_df$groups = as.factor(kmeans_clusters)
mds_df$expr = meta$expr  
...
ggscatter(mds_df, x = "V1", y = "V2",
  ...
  geom_text_repel(aes(label = expr), box.padding = 0.5)
```
