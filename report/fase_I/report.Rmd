...
bg_genes_PP = prepare_gmt("C:/Users/olive/Desktop/Extracao/report/fase_I/h.all.v2023.2.Hs.symbols.gmt", vector_PP, savefile = FALSE)


#bg_genes_TRU = prepare_gmt("~/h.all.v2023.2.Hs.symbols.gmt", vector_TRU, savefile #= FALSE)

bg_genes_TRU = prepare_gmt("C:/Users/olive/Desktop/Extracao/report/fase_I/h.all.v2023.2.Hs.symbols.gmt", vector_TRU, savefile = FALSE)
```
...
De seguida, procedeu-se à preparação do dados para a realização do PCA. Fez-se a substituição dos nomes das amostras (representando pacientes) no conjunto de dados pelos valores da coluna "expr". Pois, permitirá enriquecer a análise de PCA ao incorporar informações contextuais sobre as amostras.

```{r}
# Supondo que "dados" seja o seu conjunto de dados e "metadados" seja o dataframe de metadados

# Obter os nomes das amostras do dataframe de metadados
nomes_amostras = meta$sample_id

# Correspondência entre os nomes das amostras no conjunto de dados e no dataframe de metadados
indices_correspondentes = match(colnames(counts_keep), nomes_amostras)

# 3. Substituir os nomes das colunas pelos valores da coluna "expr" no dataframe de metadados
colnames(counts_keep) <- meta$expr[indices_correspondentes]

```


De seguida, foi efetuado o PCA através da função "prcomp()". Uma vez que esta função "prcomp" espera que as colunas sejam genes e as linhas as amostras, temos de efetuar a transporta da matriz do nosso dataset. O objetivo será formar um gráfico para perceber como é que os genes se encontram relacionados.

```{r}
data_normal = counts_keep

pca = prcomp(t(data_normal), center = F)
...
De seguida, usamos as duas primeiras colunas de "x" para criar um gráfico 2D, que usa os dois primeiros componentes principais (PC), PC1 e PC2.
...
Através da visualização do gráfico, podemos observar que 1 amostra encontra-se à esquerda e que outras duas encomtram-se mais à direita.
...
O eixo dos x diz-nos que o PC1 engloba 89.1% da variação total do conjunto de dados e o eixo do y engloba 10.9% da variação total do conjunto de dados.
...
pca_genes = data.frame(Sample=rownames(pca$x), X = pca$x[,1], Y = pca$x[,2])
