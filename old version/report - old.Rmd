...
A análise dos dados foi feita utilizando a ferramenta `edgeR`, uma biblioteca desenvolvida para o Bioconductor [@bioconductor_ da linguagem de programação Rwebsite], especialmente projetada para análise de dados de expressão genética baseada em contagens de RNA-seq. A abordagem estatística empregada pelo edgeR é baseada em métodos bayesianos, permitindo uma análise robusta e precisa dos dados de expressão genética[@edgeR_doc].
...
Para este trabalho utilizamos as seguintes bibliotecas:

-   **annotables**: oferece tabelas de anotação para uma variedade de conjuntos de dados, incluindo informações sobre genes.

-   **BiocManager**: ferramenta conveniente para instalar e atualizar pacotes do Bioconductor.
...
-   **clusterProfiler**: oferece ferramentas para realizar análises de GSEA e perfis de cluster, facilitando a interpretação de conjuntos de genes diferencialmente expressos.

-   **dplyr**: fornece uma série de funções verbais para manipulação de dados.

-   **edgeR**: ferramenta para análise de dados de RNA-seq, oferecendo métodos estatísticos avançados para identificar genes diferencialmente expressos.

-   **EnsDb.Hsapiens.v79**: base de dados de anotações gerada a partir do Ensembl.

-   **factorextra**: fornece funções para extrair e visualizar os resultados de análises de dados multivariados.

-   **fgsea**: implementação eficiente e rápida do algoritmo de análises de enriquecimento genético (GSEA).
...
-   **GSEABase**: oferece funcionalidades básicas para realizar GSEA.
...
-   **randomForest**: Classificação e regressão com base numa "floresta de árvores" utilizando entradas aleatórias.
...
-   **RSNNS**: O Stuttgart Neural Network Simulator (SNNS) é uma biblioteca que contém muitas implementações padrão de redes neurais.
...
-   **TCGAbiolinks**: fundamental para acesso e análise de dados do TCGA, permitindo insights valiosos sobre uma variedade de tipos de cancro.
...
```{r pre-processamento-meta-genero}
...
```{r pre-processamento-meta-susbtipos}
...
Abaixo é feita uma análise da frequência de idades do conjunto de dados.

```{r frequencia-idade}

idade = na.omit(luad_data$paper_Age.at.diagnosis)
idade = idade[idade != '[Not Available]']
...
idade_int = cut(idade, breaks = breaks, labels = labels, include.lowest = TRUE)


# Barplot

barplot(table(idade_int), main = "Distribuição de idades", xlab = "Intervalos de idades")

abline(h = mean(idade), col = "red", lty = 2, lwd = 1)

legend("topright", legend = paste("Idade média:", round(mean(idade), 2)), col = "red", lty = 2, lwd = 1)
```

Concluímos que a idade média da população do estudo é aproximadamente 66 anos e que é plausível inferir que o maior volume de incidência está entre os 60 e os 69 anos de idade.
...
barplot(contingency_table, beside = TRUE, legend.text = TRUE,
        main = "Estado vital vs. Estágio de Tumor",
        xlab = "", ylab = "Frequência",
        col = c("lightgreen", "pink"),
        las = 2,
        )

mtext("Estágio tumor", side = 1, line = 6) 

```

Ao analisar os resultados do teste de qui-quadrado é plausível aferir que, em função do valor p (0.008561) é plausível rejeitar a hipótese nula e inferir que existe uma correlação entre o estágio do tumor e a taxa de mortalidade. Uma análise do gráfico de barras em que é comparada a taxa de mortalidade por estágio de tumor permite também tirar esta conclusão, nomeadamente que a partir do estágio IIIA a taxa de mortalidade é relativamente superior.

# Preparação para análise de expressão diferencial

### Normalização dos dados usando contagens por milhão (CPM)

Neste processo, os dados de expressão genética são refinados antes da análise. Primeiro, é efetuado o cálculo das Contagens Por Milhão (CPM), um método de normalização de níveis de expressão comummente utilizado para análise de dados RNAseq. Este método ajusta diferenças de profundidade da sequenciação, dando dados de expressão relativos mais facilmente comparáveis entre todos os genes da amostra [@scheepbouwer2023 , @pluto_website].
...
Depois, foi feito o mapeamento dos IDs dos genes Ensembl para os seus símbolos correspondentes e substituídos os IDs pelos símbolos nos dados de expressão genética representados pelo objeto logcounts. Tornando a interpretação e visualização dos dados de expressão genética mais compreensíveis.

```{r substituicao-ensembl}
...
```{r  substituicao-ensembl-continuacao }
...
De seguida, procedeu-se à logaritmização dos dados, uma prática comum no tratamento de dados de expressão genética. A logaritmização é essencial para reduzir o enviesamento dos dados.
...
Portanto, a identificação e o estudo dos outliers são cruciais para interpretar corretamente os resultados das análises de expressão genétca e para discernir padrões biologicamente relevantes das variações aleatórias nos dados.

### Comparação de níveis de expressão para os diferentes subtipos de LUAD

Abaixo é feita a análise dos padrões de expressão genética associados a diferentes subtipos de LUAD (TRU, PI e PP) dos 30 genes com maior variabilidade detetada com recurso a um *heatmap*. A utilização de *heatmaps* é particularmente útil para análises de expressão genética uma vez que permite a identificação de padrões de expressão genética associados a condições específicas que podem dar pistas no sentido de identificação de biomarcadores para cada condição.

Abaixo é apresentado um *heatmap* para os 30 genes com maior variabilidade em termos de níveis de expressão para cada um dos subtipos de LUAD.
...
Ao analisar os heatmaps, torna-se evidente a presença de diferentes perfis de expressão genética para cada um dos subtipos de LUAD.

Conseguiu-se concluir que, por exemplo, os genes LCAL1, da família AKR1 e NR0B1são sobrexpressos no subtipo TRU. Além disso, observa-se que no subtipo PP destacam-se a sobrexpressão dos genes SLC10A2 e AC079466 (ENSG00000266976), com níveis de expressão dentro da média para os outros subtipos. No entanto, também é visível uma subexpressão do gene AKR1C2. No subtipo PI, por exemplo, destaca-se uma ligeira sobrexpressão dos genes AFAP1-AS1 e IVL
...
Adicionalmente, os genes SLC10A2 e AC079466 também mostram potencial como biomarcadores dos tipos de tumor proximal, com diferenças notáveis entre os subtipos PI e PP. O gene SLC10A2, é um responsável pela codificação de transportadores de solutos, tendo sido identificado como gene de interesse para a inibição da proliferação de cancro e promoção da aptose sob terapia com bexaroteno [@ai2018]. Uma próxima análise que seria relevante para tirar conclusões acerca destes resultados seria a análise do tipo de terapia a que os pacientes da amostra foram submetidos. O gene AC079466 aparenta desempenhar um papel importante na regulação do complexo DISC1, indiciado como potencial contribuinte para o desenvolvimento de tumores e respostas imunitárias [@zheng2023].

### Normalização por Trimmed Means of M-values
...
Em primeiro lugar, colocaram-se os dados referentes aos subtipos de expressão numa variável e, de seguida, coverteu-se a variável "meta_expr" para o tipo "caractér" para permitir futura compatibilidade com as operações efetuadas.

```{r agrupamento-variaveis}
...
No eixo do x, a média log CPM representa a abundância média dos genes nos diferentes grupos ou condições, sendo que valores mais altos indicam maior expressão média dos genes. No eixo y, o BCV mede a variabilidade biológica dos dados de expressão genética, com valores mais altos indicando maior variabilidade entre os genes.
...
Devolveu um objeto DGEGLM com os valores estimados dos coeficientes *generalized linear model* (GLM) para cada gene.
...
-   Um aumento unitário em "meta\$exprprox_prolif" (subtipo PP) está associado a uma diminuição de 0.09420212 unidades na expressão genética do gene TSPAN6. Estes dados estão de acordo com a literatura uma vez que, a baixa expressão de TSPAN6 correlaciona-se com um mau prognóstico em pacientes com cancro do pulmão [@humbert2022tspan6].

-   Um aumento unitário em "meta\$exprTRU" (subtipo TRU) está associado a um aumento de 0.4591496 unidades na expressão do gene DPM1. Este faz parte da via de glicosilação do distroglicano e foram encontradas relação entre a sua sobreexpressão e o desenvolvimento de cancro [@li2020dpm1].
...
Por exemplo, o gene ENSG00000288638 (LNCDAT) mostra uma diminuição significativa na expressão no subtipo PP, com um valor de P extremamente baixo, indicando uma forte associação estatística. Por outro lado, o gene LGALS4 exibe um aumento significativo na expressão no subtipo PP, também com um valor de P muito baixo. Este último encontra-se associado à proliferação de tumores [@das-2023]
...
Ao analisar os resultados, observamos que diversos genes, por exemplo, o gene PTGS2 apresenta uma diminuição significativa na expressão no subtipo TRU, com um valor de P muito baixo, indicando uma forte associação estatística.
...
Assim, foram realizadas duas análises de enriquecimento, uma para o conjunto de genes alvo do coeficiente "meta\$exprprox_inflam" e outra para o conjunto de genes alvo do coeficiente "meta\$exprTRU", onde seguimos este pipeline: <https://biostatsquid.com/fgsea-tutorial-gsea/>.
...
Em primeiro lugar, obtivemos um vetor com os nomes do genes provenientes dos resultados.
...
Analisando o gráfico para o coeficiente PP verificamos que foram filtrados 10 pathways, sendo portanto os únicos com expressão diferencial significativa. Desses 10, os últimos 5 pathways apresentam p-*values* bastante menores, indicando uma maior expressão diferencial. Além disso, todos estes 5 apresentam valores de NES negativos, o que significa que estão *downregulated* relativamente à condição PI. Os primeiros 5 estão *upregulated* em relação à mesma condição. Também é possível reparar que um dos *pathways* *downregulated* é o *pathway* da resposta inflamatória, o que seria de esperar visto que está a ser comparado o subtipo PP com o *Proximal Inflamatory* (PI), logo seria de esperar que o PI tivesse uma expressão muito superior dos genes deste *pathway*.
...
De seguida, foi efetuado o PCA através da função "prcomp()".

```{r}
pca = prcomp(data_normal, center = F)
```

Vamos agora averiguar quanta variação é que cada PC representa. Uma vez que a percentagem de cada quantidade de variação em cada PC é mais interessante que o próprio valor, vamos observar esses resultados aravés de uma gráfico de barras.

```{r}
fviz_eig(pca, addlabels = TRUE)
```

O primeiro componente principal contém a maior parte da variação dos dados, o segundo contém a segunda maior variação, e assim sucessivamente. Conseguimos perceber que os dois primeiros PCs contêm a variação de quase todo o dataset, sendo que o PC1 contém mais de metade de toda a variação dos dados.

Para compreender a correlação entre os subtipos de expressão e a forma como estão bem representadas pelo nosso modelo, podemos utilizar a função fviz_pca_var() para desenhar um gráfico de correlação de variáveis, utilizando o comando abaixo.
...
Podemos ver que os três tipos de subexpressão estão agrupados, o que significa que estão correlacionados entre si.

Podemos também recorrer a um gráfico de dispersão. Cada ponto no gráfico representa uma amostra, e sua posição é determinada pelas pontuações nos componentes principais (PC1 e PC2) do PCA. A cor de cada ponto corresponde ao valor da variável `expr` no dataframe de metadados.
...
Através deste gráfico de dispersão das amostras, não são observados clusters, isso pode indicar que as variáveis individuais se comportam de maneira diferente em relação à variação total dos dados.

Parece haver correlação entre os subtipos de expressão uma vez que se encontram na mesma cor. Porém, podemos ver que os subtipos encontram-se agrupados o que significam que apresentam perfis de expressão singulares apesar da aparente correlação entre eles.

Agora passando à análise dos genes para descobrir como é que as amostras se relacionam.
...
No PCA dos genes, apenas três pontos são visíveis, indicando que a variação entre eles não é tão distintiva nas primeiras duas componentes principais. Isto pode ser devido à falta de variação entre os genes ou à presença de redundância nos dados, dificultando uma separação clara em um espaço de menor dimensionalidade

A seguir, analisamos os *loading* *scores* do PC1 usando a infomação dos genes, uma vez que ele representa cerca de 90% da variação nos dados. Os *loading* *scores* representam o peso de cada gene nos PC's. Quando analisamos o PC1, estamos interessados na contribuição de cada gene para a variabilidade nos dados. Vamos examinar a magnitude usando a função abs() pois, estamos a considerar o valor absoluto dos *loading* *scores*, concentrando-nos apenas na magnitude do impacto de cada gene. Se um gene tem um *loading* *score* alto em valor absoluto então ele tem uma forte influência na direção do PC1.
...
Este conjunto de genes é o top 10 genes com maior impacto na variabilidade dos dados. Vamos agora averiguar se afetam a variabilidade do gráfico PCA 2D à esquerda ou à direita pois, um valor negativo indica que o gene empurra as amostras para a esquerda do gráfico do PCA 2D, enquanto um valor positivo indica que empurra para a direita.

```{r}
pca_gene$rotation[top_10_genes,1]
```

Sabe-se que, por exemplo, o gene MT-ND4 é um potencial biomarcador. Uma investigação recente trouxe à luz novas perspectivas sobre a influência mitocondrial na tumorigênese e metástase, destacando potenciais biomarcadores no mtDNA, especialmente o MT-ND4. Reforçando a importância de investigar a genética mitocondrial em conjunto com a progressão do cancro [@cavalcante-2022]. Outros estudos relataram amplificações dos genes CD74 e IGHG1 em larga escala em células de pacientes com LUAD [@zhang-2022, @cui-2021].

## Análise Multi Dimensional

A análise multidimensional é uma técnica de processamento de dados que permite analisar e explorar dados a partir de várias dimensões simultaneamente. Através da organização dos dados numa estrutura multidimensional chamada cubo de dados, composta por dimensões, medidas e hierarquias. Possibilitando a exploração dinâmica, análise e visualização de dados a partir de várias perspectivas [@dremio-2024].

-   **Subtipo PP**

Em primeiro lugar, selecionamos os 30 genes mais diferencialmente expressos tendo em conta os resultados relativos ao subtipo PP.
...
Depois, foi calculada a matriz de distâncias dos nossos dados de forma a medir a diferença entre cada par de observações. Permitindo entender melhor as relações entre os pontos de dados mais adiante.
...
De seguida, para a análise multidimensional recorremos à técnica clássica uma vez que é a mais adequada para dados quantitativos (expressão genética) [@unknown-author-no-date]. Usamos então a função "cmdscale".

```{r}
cmd.mds  = cmdscale(dist_data_30_pp, 3, eig = TRUE)
```

Depois foi calculada a variância explicada por cada coordenada no escalonamento multidimensional clássico e os valores foram convertidos em percentagem.
...
De seguida, foram criados rótulos para as coordenadas no espaço multidimensional, indicando a proporção da variância explicada por cada coordenada. Esses rótulos são usados posteriormente para identificar e interpretar cada dimensão no contexto da análise.
...
Depois, foi construído um gráfico para visualizar os pontos resultantes da análise multidimensional nas duas primeiras dimensões.
...
A presença de um cluster, como o que é possível observar no gráfico gerado, pode sugerir porque as amostras têm padrões de expressão genética semelhantes associadas ao subtipo PP.

-   **Subtipo TRU**

Em primeiro lugar, selecionamos os 30 genes mais diferencialmente expressos tendo em conta os resultados relativos ao subtipo TRU.
...
De seguida, usamos a função "cmdscale".
...
```{r}
lab.pc1_tru <- paste("Coordinate 1", sprintf("(var %.3f)", var.pc1_tru))
lab.pc2_tru <- paste("Coordinate 2", sprintf("(var %.3f)", var.pc2_tru))
```
...
A presença de um cluster no gráfico pode indicar uma tendência de similaridade nos padrões de expressão genética entre as amostras.

# Clustering

O clustering permite o agrupamento de objetos que estão intimamente associados dentro de um determinado conjunto de dados. Ao realizar clustering, atribuímos características (ou propriedades) a cada grupo. Em seguida, podemos proceder à criação do que chamamos de clusters com base nessas propriedades compartilhadas. É uma ferramenta útil e direta para compreender padrões de dados. Sendo também útil na identificação de anomalias ou outliers, que são casos que se destacam dos restantes dados [@hassan-2023].

## Clustering hierárquico

É um método popular para agrupar objetos através da criação de grupos de modo que os objetos dentro de um grupo sejam semelhantes entre si e diferentes dos objetos nos restantes grupos. Os clusters são representados visualmente através de um dendrograma. Possui algumas vantagens:

-   Não é necessário especificar previamente o número de clusters.
-   Os dados são facilmente resumidos/organizados usando dendrogramas.
-   Os dendrogramas facilitam a análise e interpretação dos clusters.

Para clustering hierárquico, é usada a matriz de distâncias. E depois, o algoritmo de clustering hierárquico é aplicado à matriz de distâncias para agrupar os pontos em clusters hierárquicos. Por fim, é gerado um dendrograma que visualiza a estrutura hierárquica dos clusters. Este dendrograma mostra como os clusters são formados e como os diferentes pontos de dados estão relacionados entre si em diferentes níveis de similaridade [@unknown-author-no-date1].
...
Em cima usamos o método "complete" em que é feito o cálculo da distância entre dois clusters como a maior distância entre os pares de pontos pertencentes a cada um dos dois clusters. Por outra palavras, é medida a distância máxima entre os elementos de dois clusters.

Seguidamente, experimentámos com o método "average" que calcula a distância entre dois clusters como a média das distâncias entre todos os pares de pontos pertencentes a cada um dos dois clusters.
...
seqdata = as.data.frame(assay(luad_data, 'unstranded'))
expr = luad_data$paper_expression_subtype
colnames(seqdata) = luad_data$sample_id
seqdata = seqdata[,na.omit(luad_data$paper_expression_subtype)]
...
Começamos por particionar os dados, utilizando uma fração de 80% para treino e uma fração de 20% para testes, seguindo as boas práticas geralmente aceites no que respeita a aprendizagem automática.
...
### Análise dos resultados do modelo e respetivas matrizes de confusão.

```{r}
# Carregamento dos modelos pré-treinados
#setwd('../')
setwd("C:/Users/olive/Desktop/Extracao")
...
# C4.5 decision tree
luad.c4.5.100 <- readRDS("./models/c4.5_decision_tree/luad.c4.5.100")
luad.c4.5.500   = readRDS('./models/c4.5_decision_tree/luad.c4.5.500')
luad.c4.5.1000  = readRDS('./models/c4.5_decision_tree/luad.c4.5.1000')
luad.c4.5.5000  = readRDS('./models/c4.5_decision_tree/luad.c4.5.5000')
luad.c4.5.10000 = readRDS('./models/c4.5_decision_tree/luad.c4.5.10000')


# SVM
luad.svm.100   = readRDS('./models/support_vector_machine/luad.svm.100')
luad.svm.500   = readRDS('./models/support_vector_machine/luad.svm.500')
luad.svm.1000  = readRDS('./models/support_vector_machine/luad.svm.1000')
luad.svm.5000  = readRDS('./models/support_vector_machine/luad.svm.5000')
luad.svm.10000 = readRDS('./models/support_vector_machine/luad.svm.10000')
luad.svm.19529 = readRDS('./models/support_vector_machine/luad.svm.19529')
...
# Utilizamos o modelo de inferência no conjunto de testes
pred.mlp.100 = predict(luad.mlp.100,testData[,1:100])
...
Nota: Não foi realizado o treino do modelo C4.5 decision tree para os 19529 genes devido a um erro levantado por conlitos com nomes de genes duplicados no dataset.
